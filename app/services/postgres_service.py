...
def get_table_data(table_name: str):
    db = SessionLocal()
    try:
        result = db.execute(text(f'SELECT * FROM "{table_name}" LIMIT 200'))
        return [dict(row._mapping) for row in result]
    finally:
        db.close()


#them xoa sua trong postgres dashboard

#from app.database.postgres import get_pg_connection
from app.database.postgres import get_conn as get_pg_connection

ALLOWED_TABLES = {"classes", "subject", "topic", "lessons"}
